...
namespace PARAM {
    ...
    namespace Field{
        constexpr int POS_SIDE_LEFT = 1;
        constexpr int POS_SIDE_RIGHT = -1;
        constexpr int MAX_PLAYER = 16;
        constexpr int MAX_BALL_SPEED = 6300;
        /* 长度单位均为厘米,时间单位均为秒,质量单位为克,角度单位为弧度 */
        /* Ball */
        constexpr double BALL_SIZE	= 21.5; // 半径
        constexpr double BALL_DECAY = -0.8; // 阻力对球的加速度和速度成正比,单位为 /s
        /* Player */
        constexpr double MAX_PLAYER_SIZE = 180;
        ...
        constexpr double PITCH_MARGIN = 10;                   // 边界宽度
        constexpr double CENTER_CIRCLE_R = 500;               // 中圈半径

        constexpr double GOAL_POST_AVOID_LENGTH = 20;         //伸进场地内门柱的避障长度
        constexpr double GOAL_POST_THICKNESS = 20;           //门柱宽度

        constexpr double PENALTY_MARK_X = 4800;               // 点球点的X坐标
        constexpr double OUTER_PENALTY_AREA_WIDTH = 1950;     // 外围禁区宽度(界外开球时不能站在该线内)
        constexpr double FREE_KICK_AVOID_BALL_DIST = 500;     // 开任意球的时候,对方必须离球这么远
    }
    namespace Rule{
        constexpr int Version = 2019; // 规则的版本/年份
        constexpr double MaxDribbleDist = 500; // 最大带球距离, 0代表没有限制
    }
    namespace Vision{
        constexpr double FRAME_RATE = 73;
    }
    namespace Latency{
        // 下面分开的部分是给server作仿真用的
        constexpr double TOTAL_LATED_FRAME = 4.7;
    }
    namespace Physics {
    ...
    namespace Vehicle{
        namespace V2{
            constexpr double PLAYER_SIZE = 90;
            constexpr double PLAYER_FRONT_TO_CENTER = 80.0;
            constexpr double PLAYER_CENTER_TO_BALL_CENTER = 93;
            constexpr double KICK_ANGLE = PARAM::Math::PI*30/180; // 可以击球的最大相对身体角度
            constexpr double DRIBBLE_ANGLE = PARAM::Math::PI*17/180; // 可以带球的最大相对身体角度
            constexpr double HEAD_ANGLE = 57*PARAM::Math::PI/180; // 前面的开口角度
        }
    }
    namespace AvoidDist{
        //避障所用参数
        constexpr double TEAMMATE_AVOID_DIST = PARAM::Vehicle::V2::PLAYER_SIZE*3;
        constexpr double OPP_AVOID_DIST = PARAM::Field::MAX_PLAYER_SIZE; // 厘米 18
        constexpr double BALL_AVOID_DIST = PARAM::Field::BALL_SIZE/2+20.0; // 厘米3
    }
}
namespace ZSS {
