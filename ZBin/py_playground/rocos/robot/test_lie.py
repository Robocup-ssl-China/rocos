...
pos = [opti.variable(3, 1) for _ in range(N + 1)]
vel = [opti.variable(6, 1) for _ in range(N + 1)]
dt = T / N
...
for k in range(N):
    ...
    trans_SE3 = SE3Tangent(vel[k] * dt)
    pose_SE3 = SE3(quat[k], pos[k])
    opti.subject_to(quat[k + 1] == (trans_SE3 + pose_SE3).rotation().as_quat())
